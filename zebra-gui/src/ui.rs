...
use clay::render_commands::RenderCommandConfig::{Rectangle, ScissorEnd, ScissorStart};
...
pub fn dbg_ui(ui: &mut Context, is_rendering: bool) -> bool {
    ...
    if ui.pixel_inspector_primed {
        if ui.input().mouse_pressed(MouseButton::Left) {
            unsafe {
                *ui.draw().debug_pixel_inspector = Some((
                    ui.input().mouse_pos().0.clamp(0, ui.draw().window_width) as usize,
                    ui.input().mouse_pos().1.clamp(0, ui.draw().window_height) as usize,
                ));
            }
            ui.pixel_inspector_primed = false;
        }
    ...
    if is_rendering {
        if ui.pixel_inspector_primed {
            ui.draw().text_line(
                ...
                0.0,
                16.0,
                "Pixel Inspector is Primed! Click to select pixel.",
                0xff_00ff00,
            );
        }
        if let Some((x, y)) = unsafe { *ui.draw().debug_pixel_inspector } {
            ...
            let mut draw_x = 0;
            let mut draw_y = 0;
            if x < ui.draw().window_width / 2 {
                draw_x = ui.draw().window_width - 256
            };
            if y < ui.draw().window_height / 2 {
                draw_y = ui.draw().window_height - 256
            };
...
#[derive(Debug, Copy, Clone, PartialEq, PartialOrd, Default, Hash, Ord, Eq)]
pub enum Direction {
    #[default]
    LeftToRight,
    TopToBottom,
...
impl Default for Sizing {
    fn default() -> Self {
        Self::Fit(0.0, f32::MAX)
    }
}
impl Align {
    pub const TopLeft: Self = Self {
        y: AlignY::Top,
        x: AlignX::Left,
    };
    ...
    pub const BottomLeft: Self = Self {
        y: AlignY::Bottom,
        x: AlignX::Left,
    };
    ...
    pub const BottomRight: Self = Self {
        y: AlignY::Bottom,
        x: AlignX::Right,
    };
}
...
pub fn id_index(label: &str, index: u32) -> Id {
    let id =
        unsafe { clay::Clay__HashString(label.into(), index, clay::Clay__GetParentElementId()) };
    Id {
        id: id.id,
        offset: id.offset,
        base_id: id.baseId,
...
pub const Clay_ElementId_ZERO: clay::Clay_ElementId = clay::Clay_ElementId {
    id: 0,
    offset: 0,
    baseId: 0,
    stringId: clay::Clay_String {
...
pub const Clay_SizingMinMax_ZERO: clay::Clay_SizingMinMax = clay::Clay_SizingMinMax {
    min: 0f32,
    max: f32::MAX,
};
pub const Clay_SizingAxis_ZERO: clay::Clay_SizingAxis = clay::Clay_SizingAxis {
    size: clay::Clay_SizingAxis__bindgen_ty_1 {
        minMax: Clay_SizingMinMax_ZERO,
    },
    type_: clay::Clay__SizingType_CLAY__SIZING_TYPE_FIT,
};
pub const Clay_Sizing_ZERO: clay::Clay_Sizing = clay::Clay_Sizing {
    width: Clay_SizingAxis_ZERO,
    height: Clay_SizingAxis_ZERO,
};
pub const Clay_Padding_ZERO: clay::Clay_Padding = clay::Clay_Padding {
    left: 0,
    right: 0,
    top: 0,
    bottom: 0,
};
pub const Clay_ChildAlignment_ZERO: clay::Clay_ChildAlignment = clay::Clay_ChildAlignment {
    x: 0 as _,
    y: 0 as _,
};
...
pub const Clay_Color_ZERO: clay::Clay_Color = clay::Clay_Color {
    r: 0.0,
    g: 0.0,
    b: 0.0,
    a: 0.0,
};
pub const Clay_CornerRadius_ZERO: clay::Clay_CornerRadius = clay::Clay_CornerRadius {
    topLeft: 0f32,
    topRight: 0f32,
    bottomLeft: 0f32,
    bottomRight: 0f32,
};
pub const Clay_ElementDeclaration_ZERO: clay::Clay_ElementDeclaration =
    clay::Clay_ElementDeclaration {
        ...
        custom: clay::Clay_CustomElementConfig {
            customData: std::ptr::null_mut(),
        },
        clip: clay::Clay_ClipElementConfig {
            horizontal: false,
            vertical: false,
            childOffset: clay::Clay_Vector2 { x: 0f32, y: 0f32 },
        },
    ...
    fn decl(&mut self, item: Decl) -> &mut Self {
        ...
        match item.floating {
            Floating::Parent => {
                ...
                decl.floating.clipTo =
                    clay::Clay_FloatingClipToElement_CLAY_CLIP_TO_ATTACHED_PARENT;
        ...
        decl.layout.childAlignment = clay::Clay_ChildAlignment {
            x: item.align.x as _,
            y: item.align.y as _,
        };
...
pub const fn clay_colour(colour: (u8, u8, u8, u8)) -> clay::Color {
    clay::Color::rgba(
        colour.0 as f32,
        colour.1 as f32,
        colour.2 as f32,
...
impl HSVA_RGBA for (u8, u8, u8, u8) {
    #[inline(always)]
    fn hsva(self) -> Self {
        rgba_to_hsva(self.0, self.1, self.2, self.3)
    }
    #[inline(always)]
    fn rgba(self) -> Self {
        hsva_to_rgba(self.0, self.1, self.2, self.3)
    }
}
...
impl Context {
    ...
    pub fn draw(&self) -> &DrawCtx {
        unsafe { &*self.draw }
    }
    pub fn input(&self) -> &InputCtx {
        unsafe { &*self.input }
    }
    pub fn clay(&self) -> &mut Clay {
        unsafe { &mut *self.clay }
    }

    pub fn scale(&self, size: f32) -> f32 {
        (size * self.scale).floor()
    }
    pub fn scale32(&self, size: f32) -> u32 {
        self.scale(size) as u32
    }
    pub fn scale16(&self, size: f32) -> u16 {
        self.scale(size) as u16
    }

    pub fn hovered(&self, id: Id) -> bool {
        unsafe { clay::Clay_PointerOver(id.clay().id) }
    }
    ...
    pub fn button_ex(
        ...
        id: Id,
        enabled: bool,
        pointer_on_hover: winit::window::CursorIcon,
    ) -> (bool, (u8, u8, u8, u8), (u8, u8, u8, u8)) {
        ...
        let mouse_pressed =
            mouse_hover && self.input().mouse_pressed(winit::event::MouseButton::Left);
        let mouse_released =
            mouse_hover && self.input().mouse_released(winit::event::MouseButton::Left);
        ...
        if key_pressed {
            self.key_pressed_id = id;
        }
        let mouse_activated = enabled
            && self.mouse_pressed_id == id
            && if act_on_press {
                mouse_pressed
            } else {
    ...
    pub fn button(&mut self, id: Id) -> (bool, (u8, u8, u8, u8), (u8, u8, u8, u8)) {
        return self.button_ex(
            true,
            BUTTON_GREY,
            id,
            true,
    ...
    pub fn textbox(&mut self, data: &mut UiData, id: Id) -> String {
        ...
        let (activated, colour, text_colour) =
            self.button_ex(true, BUTTON_GREY, id, true, winit::window::CursorIcon::Text);
        ...
        let text = {
            ...
            if self.nav_id == id.id {
                ...
                textbox_state.selection.0 =
                    textbox_state.selection.0.min(textbox_state.text_buf.len());
                textbox_state.selection.1 =
                    textbox_state.selection.1.min(textbox_state.text_buf.len());
                ...
                let cut = (ctrl && self.input().key_pressed(KeyCode::KeyX))
                    || (shift && self.input().key_pressed(KeyCode::Delete));
                ...
                if (backspace || delete || should_cut || has_input) {
                    ...
                    let input_len = if let Some(input) = &self.input().text_input {
                        input.len()
                    } else {
                        0
                    };
                    ...
                    let mut new_buf =
                        Vec::with_capacity(pre_slice.len() + input_len + pst_slice.len());
...
pub trait Dup2: Copy {
    fn dup2(self) -> (Self, Self);
}
impl<T: Copy> Dup2 for T {
    fn dup2(self) -> (Self, Self) {
        (self, self)
    }
}
pub trait Dup3: Copy {
    fn dup3(self) -> (Self, Self, Self);
}
impl<T: Copy> Dup3 for T {
    fn dup3(self) -> (Self, Self, Self) {
        (self, self, self)
    }
}
pub trait Dup4: Copy {
    fn dup4(self) -> (Self, Self, Self, Self);
}
impl<T: Copy> Dup4 for T {
    fn dup4(self) -> (Self, Self, Self, Self) {
        (self, self, self, self)
    }
}
...
impl<T: AsF32> Mul for T {
    #[inline(always)]
    fn mul(self, f: f32) -> Self {
        T::from_f32(self.to_f32() * f)
    }
}
impl<A: Mul, B: Mul> Mul for (A, B) {
    #[inline(always)]
    fn mul(self, f: f32) -> Self {
        (self.0.mul(f), self.1.mul(f))
    }
}
impl<A: Mul, B: Mul, C: Mul> Mul for (A, B, C) {
    #[inline(always)]
    fn mul(self, f: f32) -> Self {
        (self.0.mul(f), self.1.mul(f), self.2.mul(f))
    }
}
impl<A: Mul, B: Mul, C: Mul, D: Mul> Mul for (A, B, C, D) {
    #[inline(always)]
    fn mul(self, f: f32) -> Self {
        (self.0.mul(f), self.1.mul(f), self.2.mul(f), self.3.mul(f))
    }
}
...
pub fn ui_left_pane(
            ...
            let title_bar = |ui: &mut Context, closeable, title, title_bar_id| {
                if let _ = elem().decl(Decl {
                    ...
                    }) && closeable
                    {
                        ...
                        let (clicked, colour, _) = ui.button_ex(
                            false,
                            BUTTON_GREY,
                            id,
                            true,
                        ...
                        if ui.hovered(container_id)
                            && !ui.hovered(contents_id)
                            && ui.input().mouse_pressed(winit::event::MouseButton::Left)
                        {
                            ui.modal = Modal::None;
            ...
            match ui.modal {
                ...
                Modal::Send => {
                    ...
                    if let _ = elem().decl(Decl {
                        ...
                        if (wallet_state.lock().unwrap().balance as u64) < ONE_cTAZ / 100 {
                            ...
                            ui.text(
                                "Insufficient funds. Try the faucet!",
                                TextDecl {
                                    h: ui.scale(20.0),
                                    colour,
                                    align: AlignX::Center,
                        ...
                        if let _ = elem().decl(Decl {
                            ...
                            let (balance, waiting_for_send) = {
                                let wallet_state = wallet_state.lock().unwrap();
                                (wallet_state.balance, wallet_state.waiting_for_send)
                            };

                            let can = !waiting_for_send && data.send_address.len() != 0;
                            if button_ex(ui, "1 cTAZ", can && (balance as u64) >= ONE_cTAZ) {
                                wallet_state
                                    .lock()
                                    .unwrap()
                                    .send_to_address(data.send_address.clone(), ONE_cTAZ);
                            }
                            if button_ex(ui, "10 cTAZ", can && (balance as u64) >= ONE_cTAZ * 10) {
                                wallet_state
                                    .lock()
                                    .unwrap()
                                    .send_to_address(data.send_address.clone(), ONE_cTAZ * 10);
                            }
                        }
                    }
                }
                ...
                Modal::Stake => {
                    ...
                    ui.text(
                        frame_strf!(
                            ...
                            "[{}..{}]",
                            &stake_address[0..8],
                            &stake_address[stake_address.len() - 8..]
                        ),
                        TextDecl {
                            font: Mono,
                            h: ui.scale(20.0),
                    ...
                    const ONE_cTAZ: u64 = 100_000_000;
                    let waiting_for_stake_to_finalizer =
                        wallet_state.lock().unwrap().waiting_for_stake_to_finalizer;

                    {
                        ...
                        if button_ex(ui, "+0.01 cTAZ", can && (balance as u64) >= ONE_cTAZ / 100) {
                            wallet_state
                                .lock()
                                .unwrap()
                                .stake_to_finalizer(ONE_cTAZ / 100, hex_dest.unwrap());
                        }
                        if button_ex(ui, "+0.1 cTAZ", can && (balance as u64) >= ONE_cTAZ / 10) {
                            wallet_state
                                .lock()
                                .unwrap()
                                .stake_to_finalizer(ONE_cTAZ / 10, hex_dest.unwrap());
                        }
                        if button_ex(ui, "+1 cTAZ", can && (balance as u64) >= ONE_cTAZ) {
                            wallet_state
                                .lock()
                                .unwrap()
                                .stake_to_finalizer(ONE_cTAZ, hex_dest.unwrap());
                        }
                        if button_ex(ui, "+10 cTAZ", can && (balance as u64) >= ONE_cTAZ * 10) {
                            wallet_state
                                .lock()
                ...
                Modal::Unstake => {
                    ...
                    let mut clickable_icon = |ui: &mut Context, id, icon, icon_hovered, enabled| {
                        let (clicked, colour, _) = ui.button_ex(
                            true,
                            (0xcc, 0xcc, 0xcc, 0xff), /* @todo colors */
                            id,
                            enabled,
                    ...
                    let scroll_container_data: clay::Clay_ScrollContainerData =
                        unsafe { clay::Clay_GetScrollContainerData(id.clay().id) };
                    ...
                    if let _ = elem().decl(Decl {
                        ...
                        if staked_roster.len() == 0 {
                            ...
                            if let _ = elem().decl(Decl {
                                ...
                                ui.text(
                                    "You have not staked to any finalizers yet.",
                                    TextDecl {
                                        colour: WHITE.mul(0.6),
                                        h,
                                        align: AlignX::Center,
                            ...
                            for (index, member) in staked_roster.iter().enumerate() {
                                ...
                                if let _ = elem().decl(Decl {
                                    ...
                                    if let _ = elem().decl(Decl {
                                        id: id_index(
                                            "Unstake Roster Member Left Icon",
                                            index as u32,
                                        ),
                                        height: fit!(),
                                        ...
                                        if clickable_icon(
                                            ui,
                                            id_index("Unstake Button", index as u32),
                                            ICON_LINK_1,
                                            ICON_UNLINK,
                                            true,
                                        ) {
                                            wallet_state
                                                .lock()
                                                .unwrap()
                                                .unstake_from_finalizer(member.1);
                                        }
                                    }
                                    ...
                                    if let _ = elem().decl(Decl {
                                        ...
                                        ui.text(
                                            frame_strf!(data, "{}", display_str(&chunks)),
                                            TextDecl {
                                                font: Mono,
                                                h: ui.scale(14.0),
                                                align: AlignX::Left,
                                    ...
                                    if let _ = elem().decl(Decl {
                                        ...
                                        let mut str = frame_strf!(
                                            data,
                                            "{}.{} cTAZ",
                                            full,
                                            &part_str[..trim_part.len().max(3)]
                                        );
                                        if ui.hovered(id) {
    ...
    if let _ = elem().decl(Decl {
        ...
        tab_id_wallet = ui.tab(
            (radius.0, 0.0, radius.2, radius.3),
            padding,
            tab_id,
            "Wallet",
        );
    ...
    if let _ = elem().decl(Decl {
        ...
        let (balance, pending_balance, staked_balance, show_staked_balance) = {
            let wallet_state = wallet_state.lock().unwrap();
            (
        ...
        if let _ = elem().decl(Decl {
            ...
            let suffix = if !show_staked_balance && staked_balance > 0 {
                "*"
            } else {
                ""
            };
            let balance_str = frame_strf!(
                data,
                "{} cTAZ{}",
                str_from_ctaz(balance.try_into().unwrap()),
                suffix
            );
            ui.text(
                &balance_str,
        ...
        if let _ = elem().decl(Decl {
            ...
            let balance_str = frame_strf!(
                data,
                "{} cTAZ Pending",
                str_from_ctaz(pending_balance.try_into().unwrap())
            );
            ui.text(
                &balance_str,
                TextDecl {
                    h: accent_text_h,
                    align: AlignX::Center,
                    colour: (0x90, 0x90, 0x90, 0xff), /* @todo colors */
                    ..TextDecl
                },
            );
        }
        ...
        if let _ = elem().decl(Decl {
            ...
            if button(ui, ICON_UP_BIG, "Send") {
                ui.modal = Modal::Send;
            }
            if button(ui, ICON_QRCODE, "Receive") {
                ui.modal = Modal::Receive;
            }
            if button(ui, ICON_LINK_1, "Stake") {
                ui.modal = Modal::Stake;
            }
            if button(ui, ICON_UNLINK, "Unstake") {
                ui.modal = Modal::Unstake;
            }
        }
                            ...
                            if let _ = elem().decl(Decl {
                                ...
                                match tx.kind() {
                                    ...
                                    | WalletTxKind::Stake => {
                                        ...
                                        let prefix = if tx.kind() == WalletTxKind::Send {
                                            "-"
                                        } else {
                                            ""
                                        };
                                        ...
                                        if true {
                                            ...
                                            ui.text(
                                                frame_strf!(
                                                    data,
                                                    "+{} cTAZ",
                                                    str_from_ctaz(tx.totals().recv_zats.into_u64())
                                                ),
                                                TextDecl {
                                                    h: transaction_text_h,
                                                    align: AlignX::Right,
                                            ...
                                            let (t_z, s_z) = (
                                                tx.parts[0].recv_zats.into_u64(),
                                                tx.parts[1].recv_zats.into_u64(),
                                            );
                                            ui.text(
                                                frame_strf!(
                                                    data,
...
pub fn ui_right_pane(
    ...
    if let _ = elem().decl(Decl {
        ...
        tab_id_roster = ui.tab_ex(
            (0.0, radius.1, radius.2, radius.3),
            padding,
            tab_id,
            id("Finalizers"),
            frame_strf!(data, "Finalizers ({})", roster.len()),
        );
    }
    if let _ = elem().decl(Decl {
        ...
        if let _ = elem().decl(Decl {
            ...
            ui.text(
                frame_strf!(
                    ...
                    "[{}..{}]",
                    &recv_address[0..8],
                    &recv_address[recv_address.len() - 8..]
                ),
                TextDecl {
                    font: Mono,
                    h: ui.scale(20.0),
                ...
                for (index, member) in roster.iter().enumerate() {
                    ...
                    if let _ = elem().decl(Decl {
                        ...
                        if let _ = elem().decl(Decl {
                            ...
                            if clickable_icon(
                                ui,
                                id_index("Copy Button", index as u32),
                                ICON_DOCS_1,
                                true,
                            ) {
    ...
    if let _ = elem().decl(Decl {
        ...
        tab_id_faucet = ui.tab_ex(
            ...
            frame_strf!(
                data,
                "Faucet (Height {})",
                &wallet_state.lock().unwrap().miner_seen_h
            ),
        );
    }
    if let _ = elem().decl(Decl {
        ...
        if let _ = elem().decl(Decl {
            ...
            let left_text = TextDecl {
                h: text_h,
                align: AlignX::Left,
                ..TextDecl
            };
            let right_text = TextDecl {
                font: Mono,
                align: AlignX::Right,
                ..left_text
            };
            ...
            if let _ = elem().decl(row) {
                if let _ = elem().decl(left) {
                    ui.text("Unshielded:", left_text);
                }
                if let _ = elem().decl(right) {
                    ui.text(frame_strf!(data, "{} cTAZ", str_from_ctaz(un)), right_text);
                }
            }
            if let _ = elem().decl(row) {
                if let _ = elem().decl(left) {
                    ui.text("Shielded (Pending):", left_text);
                }
                if let _ = elem().decl(right) {
                    ui.text(
                        frame_strf!(data, "{} cTAZ", str_from_ctaz(sh_p)),
                        right_text,
                    );
                }
            }
            if let _ = elem().decl(row) {
                if let _ = elem().decl(left) {
                    ui.text("Shielded (Spendable):", left_text);
                }
                if let _ = elem().decl(right) {
                    ui.text(
                        frame_strf!(data, "{} cTAZ", str_from_ctaz(sh_s)),
                        right_text,
                    );
                }
            }
        ...
        if let _ = elem().decl(Decl {
            ...
            if button_ex(
                "Receive cTAZ",
                false,
                !wallet_state.lock().unwrap().waiting_for_faucet,
            ) {
...
pub fn run_ui(
    ...
    let (window_w, window_h) = (
        ui.draw().window_width as f32,
        ui.draw().window_height as f32,
    );
    let mouse_pos = (
        ui.input().mouse_pos().0 as f32,
        ui.input().mouse_pos().1 as f32,
    );
    ...
    clay.set_measure_text_function_user_data(ui.draw(), |string, text_config, draw| {
        let font_kind = match text_config.font_id {
            0 => FontKind::Normal,
            1 => FontKind::Mono,
            2 => FontKind::Icons,
            _ => todo!(),
    ...
    if let _ = elem().decl(Decl {
        id: id("Main"),
        padding: (0.0, 0.0, padding.2, padding.3),
        child_gap,
        width: grow!(),
        height: grow!(),
        ..Decl
        ...
        if let _elem = elem().decl(Decl {
            ...
            if let _elem = elem().decl(Decl {
                ...
                if let _ = elem().decl(Decl {
                    ...
                    ui.text(
                        frame_strf!(data, "PoS Height: {}", viz.bft_tip_height),
                        TextDecl {
                            h: ui.scale(16.0),
                            align: AlignX::Center,
                            ..TextDecl
                    ...
                    ui.text(
                        frame_strf!(data, "PoW Height: {}", viz.bc_tip_height),
                        TextDecl {
                            h: ui.scale(16.0),
                            align: AlignX::Center,
                            ..TextDecl
                    ...
                    ui.text(
                        frame_strf!(data, "Orchard Pool Zatoshis: {}", viz.orchard_pool_balance),
                        TextDecl {
                            h: ui.scale(16.0),
                            align: AlignX::Center,
                            ..TextDecl
                    ...
                    ui.text(
                        frame_strf!(
                            data,
                            "Staking (Bonded) Pool Zatoshis: {}",
                            viz.staking_bonded_pool_balance
                        ),
                        TextDecl {
                            h: ui.scale(16.0),
                            align: AlignX::Center,
                    ...
                    ui.text(
                        frame_strf!(
                            data,
                            "Staking (Unbonded) Pool Zatoshis: {}",
                            viz.staking_unbonded_pool_balance
                        ),
                        TextDecl {
                            h: ui.scale(16.0),
                            align: AlignX::Center,
            ...
            if let _ = elem().decl(Decl {
                ...
                let enabled =
                    viz.camera_x != 0.0 || viz.camera_y != viz.bc_tip_y || viz.zoom != 0.0;
        ...
        // Lots of finalizers in the list pushes the faucet section outside the screen
        // This seems to fix it
        let right_id = id("Right Pane");

        if ui.hovered(right_id) {
            ui.finalizers_scroll -= ui.input().zoom_delta as f32 * 32.0;
            ui.finalizers_scroll -= ui.input().scroll_delta.1 as f32 * 32.0;
        }
        if ui.finalizers_scroll < 0.0 {
        ...
        let scroll_data: clay::Clay_ScrollContainerData =
            unsafe { clay::Clay_GetScrollContainerData(right_id.clay().id) };
        if scroll_data.found {
            let max = scroll_data.contentDimensions.height / ui.scale - 96.0;
            if ui.finalizers_scroll > max {
                ui.finalizers_scroll = max;
            }
        }
    ...
    if viz.inspecting_block_hash != Hash32::from_u64(0) {
        ...
        if let _ = elem().decl(Decl {
            ...
            if let _ = elem().decl(Decl {
                ...
                ui.text(
                    frame_strf!(data, "Block: {}", viz.inspecting_block_hash),
                    TextDecl {
                        font: Mono,
                        wrap_chars: true,
                        h: text_h,
                ...
                let text = {
                    if let Some(text) = viz.inspect_block_json_text.as_ref() {
                        ...
                        frame_strf!(
                            data,
                            "Loading info for block {}...",
                            viz.inspecting_block_hash
                        )
                        .to_string()
                    }
                };
                ui.text(
                    frame_strf!(data, "{}", text),
                    TextDecl {
                        font: Mono,
                        wrap_chars: true,
                        h: text_h,
    ...
    if is_rendering {
        ...
        for command in render_commands {
            ...
            match command.config {
                Rectangle(config) => {
                    ...
                    ui.draw().rounded_rectangle(
                        x1, y1, x2, y2, radius_tl, radius_tr, radius_bl, radius_br, colour,
                    );
                }
                RenderCommandConfig::Text(config) => {
                    let font_kind = match config.font_id {
                        0 => FontKind::Normal,
                        1 => FontKind::Mono,
                        2 => FontKind::Icons,
                        _ => todo!(),
                ...
                misc => {
                    todo!("Unsupported clay render command: {:?}", misc)
                }
            }

            if let Some(textbox_state) = data.textboxes.get_mut(&command.id)
                && ui.nav_id == command.id
            {
                ...
                let compute_x = |idx: usize| {
                    ...
                    let xoff = ui
                        .draw()
                        .measure_text_line(FontKind::Normal, textbox_state.h, &str);
                ...
                if textbox_state.selection.1 == textbox_state.selection.0 {
                    ...
                    let (min, max) = (head_x.min(tail_x), head_x.max(tail_x));
                    ...
                    ui.draw()
                        .rectangle(x1 as f32, y1 as f32, x2 as f32, y2 as f32, color);
                }
            }
        ...
        if ui.nav_enable
            && let Some((x1, y1, x2, y2)) = nav_bbox
            && !data.textboxes.contains_key(&ui.nav_id)
        {
...
pub fn ui_update(
    ...
    let result = run_ui(ui, wallet_state.clone(), data, viz, false);
    ui.input = &dummy_input;
    let result = result || run_ui(ui, wallet_state.clone(), data, viz, true);
    ui.input = real_input;
    return result;
}
