...
// Ironwood notes use their own note-encryption domain: the plaintext lead byte is 0x03
// (NoteVersion::V3), and `OrchardDomain` accepts only 0x02, so decrypting an Ironwood
// note with the Orchard domain silently yields nothing. Crosslink is Ironwood-native,
// so every trial decryption here uses `IronwoodDomain`.
use orchard::note_encryption::{ CompactAction as OrchardCompactAction, IronwoodDomain };
...
impl BuildPrep {
    fn fee_required(&self) -> Result<Zatoshis, builder::FeeError<zip317::FeeError>> {
        ...
        let orchard_actions = orchard::builder::BundleType::DEFAULT.num_actions(
            // Crosslink spends from the Orchard pool; NU6.3 onward fixes its bundle version.
...
impl<'a> TxOptions<'a> {
    // TODO: reconsider
    // pub const DEFAULT_SRC_POOLS: [TxPool; 2] = [TxPool::Sapling, TxPool::Ironwood];
    // pub const DEFAULT_SRC_POOLS: [TxPool; 1] = [TxPool::Ironwood];
}
...
impl ManualWallet {
    ...
    fn build_tx_from_prep<P: Parameters>(&mut self, network: P, tx: &mut ProposedTx, prep: BuildPrep) -> bool {
        ...
        // Upstream now takes the whole `TransparentInputInfo` (which already carries its
        // `SpendInfo`, P2PKH or P2SH) and is infallible: validation moved into
        // `TransparentInputInfo::from_parts`, which the caller has already been through.
        for t_input in t_inputs {
            txb.add_transparent_input(t_input);
        }
    ...
    ) -> Option<()>
    {
        ...
        let mut prep = BuildPrep {
            build_config: BuildConfig::Standard {
                sapling_anchor,
                // Crosslink is Ironwood-native: from NU6.3 the Orchard pool mandates the
                // cross-address restriction (ZIP 229) and so can only hold wallet-controlled
                // change, while a coinbase Orchard bundle cannot be built at all. Every value
                // this wallet moves -- mining rewards, faucet payments, delegation bonds --
                // therefore lives in the Ironwood pool, and no Orchard bundle is constructed.
                orchard_anchor: None,
                ironwood_anchor: Some(orchard_anchor),
        ...
        let mut txb = TxBuilder::new(
            ...
            // `BuildConfig` gained the Ironwood anchor/padding fields and is no longer `Copy`.
            prep.build_config.clone(),
        );
        ...
        for output in outputs {
            match output {
                ...
                &TxOutput::Ironwood{ ref ovk, dst, zats, memo: ref note_memo } => {
                    s.sent(zats, true)?;
                    ...
                    if let Err(err) = txb.add_ironwood_output::<zip317::FeeError>(ovk.clone(), dst.clone(), zats, note_memo.clone()) {
                        println!("tx build error: {err:?}");
                        return None;
                    }
        ...
        fn calc_fee<P: Parameters>(txb: &TxBuilder<P, ()>, prep: &BuildPrep) -> Option<u64> {
            let prep_fee = prep.fee_required();
                        ...
                        let res = match utxo.t_addr {
                            TransparentAddress::PublicKeyHash(_) => {
                                TransparentInputInfo::from_parts(
                                    utxo.id.clone(),
                                    utxo.txout(),
                                    SpendInfo::P2pkh { pubkey: t_pubkey },
                                )
                                .map(|input| txb.add_transparent_input(input))
                            }
                ...
                TxPool::Ironwood(tree) => {
                    if let Some(fvk) = &keys.orchard_fvk {
                        ...
                        for &note in &shuffled_notes {
                            ...
                            if let Err(err) = txb.add_ironwood_spend::<zip317::FeeError>(fvk.clone(), note.note, merkle_path.clone()) {
        ...
        let change = match total_spend.cmp(&min_spend) {
            ...
            std::cmp::Ordering::Greater => {
                ...
                if let Some(ovk) = keys.orchard_ovk {
                    s.sent(change, true)?;
                    s.recv(change, true)?;
                    if let Err(err) = txb.add_ironwood_output::<zip317::FeeError>(Some(ovk.clone()), orchard_addr.clone(), change, MemoBytes::empty()) {
    ...
    pub fn begin_unbonding_using_ironwood<P: Parameters>(
        &mut self, network: P, tx: &mut ProposedTx, client: &mut CompactTxStreamerClient<Channel>,
    ...
    pub async fn claim_bond_using_ironwood<P: Parameters>(
        &mut self, network: P, tx: &mut ProposedTx, client: &mut CompactTxStreamerClient<Channel>,
        src_usk: &UnifiedSpendingKey, orchard_tree: &OrchardShardTree, bond_key: [u8; 32],
    ) -> Option<()>
    {
...
fn read_compact_tx(wallet: &mut ManualWallet, account_i: usize, keys: &PreparedKeys, block_h: BlockHeight, tx: &CompactTx, next_orchard_pos: &mut u64, insert_i: &mut usize, orchard_tree: &mut OrchardShardTree) -> (TxId, bool/*ours*/, bool/*ok*/) {
    let txid = TxId::from_bytes(<[u8;32]>::try_from(&tx.txid[..]).expect("successfully converted above"));

    let mut shielded_part = WalletTxPart::ZERO;

    // Ironwood-native: our notes live in the Ironwood bundle (see the build config).
    for orchard_action in &tx.ironwood_actions {
...
pub async fn wallet_main(wallet_state: Arc<Mutex<WalletState>>) {
    ...
    *FAUCET_REQUEST.lock().unwrap() = Some(FaucetRequestClosure(Arc::new(|ua_str: String| -> Result<u64, String> {
    ...
    fn block_rng_from_heights(heights: (u64, u64)) -> BlockRange {
        BlockRange {
            // Empty means the server's default: all shielded pools.
            pool_types: Vec::new(),
    ...
    'outer_sync: loop {
                ...
                for block_i in first_new_block_i..new_blocks.len() {
                    ...
                    for tx in &new_blocks[block_i].vtx {
                        if <[u8;32]>::try_from(&new_blocks[block_i].hash[..]).is_err() {
                            ...
                            println!("invalid hash for compact tx at height {}: {:?}", new_blocks[block_i].height, tx.txid);
                            data_is_invalid = true;
                            break;
                        }
                    }
        ...
        if AUTO_SPEND {
            if /*user_wallet.accounts[0].unspent_orchard_notes.len() == 0 &&*/ !proposed_faucet.is_in_progress() {
                ...
                let ok = miner_wallet.send_ironwood_to_ironwood_zats(network, &mut proposed_faucet, &mut client, &miner_usk, FAUCET_VALUE, &orchard_tree, *user_ua.orchard().unwrap(),
                    MemoBytes::from_bytes("auto-sent from miner".as_bytes()).unwrap()
                ).is_some();
            } else if ! auto_spend.0 {
                // auto_spend.0 = true;
                // proposed_stake = user_wallet.stake_ironwood_to_finalizer(network, &mut client, &user_usk, 100_000_000, &orchard_tree, [0xcd;32]);
            }
        }
        ...
        loop {
                    ...
                    if ! proposed_faucet.is_in_progress() {
                        ...
                        if q.len() > 0 {
                            ...
                            if let Some(orchard_addr) = q.data[i] {
                                ...
                                let ok = miner_wallet.send_ironwood_to_ironwood_zats(network, &mut proposed_faucet, &mut client, &miner_usk, FAUCET_VALUE, &orchard_tree, orchard_addr, memo).is_some();
            ...
            let ok: bool = match &action {
                &WalletAction::RequestFromFaucet => {
                    let memo = MemoBytes::from_bytes("With love from your favourite faucet... Don't spend it all at once!".as_bytes()).unwrap();
                    let ok = miner_wallet.send_ironwood_to_ironwood_zats(network, &mut proposed_faucet, &mut client, &miner_usk, FAUCET_VALUE, &orchard_tree, *user_ua.orchard().unwrap(), memo).is_some();
                ...
                &WalletAction::StakeToFinalizer(amount, target_finalizer) => {
                    let ok = user_wallet.stake_ironwood_to_finalizer(network, &mut proposed_stake, &mut client, &user_usk, amount.into_u64(), &orchard_tree, target_finalizer).is_some();
                ...
                &WalletAction::UnstakeFromFinalizer(txid) => {
                    let ok = user_wallet.begin_unbonding_using_ironwood(network, &mut proposed_stake, &mut client, &user_usk, &orchard_tree, *txid.as_ref()).is_some();
                    just_init_new_tx |= ok;
                ...
                &WalletAction::ClaimBond(txid) => {
                    let ok = user_wallet.claim_bond_using_ironwood(network, &mut proposed_stake, &mut client, &user_usk, &orchard_tree, *txid.as_ref()).await.is_some();
                    just_init_new_tx |= ok;
