...
use crate::transaction::{
    StakingAction, StakingActionKind, Transaction, TxVersion,
...
pub struct Builder<P, U> {
    ...
    // NOTE: upstream removed `tze_builder` from `Builder` along with the `'a` lifetime it
    // required. The zfuture TZE experiment is not used by Crosslink, so the field is dropped
    // here rather than reintroducing a lifetime parameter to carry it.
...
impl<P: consensus::Parameters, U: sapling::builder::ProverProgress> Builder<P, U> {
        ...
        OP: OutputProver,
    {
        ...
        // Consistency checks
        //
        // Crosslink: a staking transaction is built as VCrosslink and currently pays no fee.
        // @todo(crosslink): "TODO(Sam): FIX FEES" -- staking actions are not yet charged.
        let fee = if self.staking_action.is_none() {
            fee
        } else {
            self.tx_version = TxVersion::VCrosslink;
            Some(Zatoshis::const_from_u64(0))
        };
        ...
        if let Some(fee) = fee {
            ...
            match balance_after_fees.cmp(&ZatBalance::zero()) {
                ...
                Ordering::Greater => {
                    // @todo(crosslink): upstream returns `Err(Error::ChangeRequired(..))` here.
                    // Crosslink disabled it ("TODO(Sam): FIX FEES") and callers currently rely on
                    // an over-funded build succeeding. Left disabled deliberately: re-enabling it
                    // is a runtime behaviour change in the wallet, not a merge decision.
                }
                Ordering::Equal => (),
            };
        }
