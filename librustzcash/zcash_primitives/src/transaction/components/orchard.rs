...
/// Reads the Orchard-protocol bundle of a Crosslink (v7) transaction.
///
/// A Crosslink transaction carries the mainnet v6 body, so its two Orchard-protocol slots use
/// the same bundle versions as v6: the Orchard slot and the Ironwood slot. `pool` selects which.
pub fn read_vcrosslink_bundle<R: Read>(
    reader: R,
    consensus_branch_id: BranchId,
    pool: ValuePool,
) -> io::Result<Option<orchard::Bundle<Authorized, ZatBalance>>> {
    read_v6_bundle(reader, consensus_branch_id, pool)
}
...
pub fn write_vcrosslink_bundle<W: Write>(
    ...
    // Must mirror `read_vcrosslink_bundle`, which reads the *v6* bundle format. Writing v5 here
    // made every VCrosslink transaction unparseable by its own reader: the v5 body has no NU6.3
    // flag byte, so the reader consumed the following field's bytes as Orchard action data and
    // failed with "Orchard action rk must not be the identity point".
    write_v6_bundle(bundle, writer)
}
