...
//! verification. The Halo2 Orchard verifier ([`super::halo2`]) and the Sapling verifier
//! ([`super::sapling`]) share it, and key their entries the same way.
//!
//! Ported from Zakura (`zakura-consensus/src/primitives/cache.rs`, PRs #597 and #600), reduced
//! to the two shielded pools this node verifies. Zakura partitions its caches by Orchard circuit
//! era and adds an Ironwood pool; crosslink has one Orchard verifying key and no Ironwood, so one
//! cache per pool is enough.
//!
...
//! removed it as a security fix (Zebra PR #10494). Transaction validity depends on height, block
//! time and spent outputs, and that cache's key named none of them.
//!
...
//! spent outputs, and skips only the proof and signature checks.
//!
...
///
/// Each key is a transaction ID, sighash and pool tag, held twice — once in the lookup set and
/// once in the eviction queue — so a full cache costs a few MiB, and roughly twice that across
/// the Orchard and Sapling caches. Only the pools a node actually verifies pay it, because each
/// cache is built with its verifier on first use.
...
///
/// One transaction can have both a Sapling bundle and an Orchard bundle under one transaction ID
/// and one sighash, so the key names which one it stands for. Each pool has its own cache here,
/// so this tag is defence in depth rather than the only thing keeping them apart.
...
///
/// Crosslink's v7 `VCrosslink` transactions carry shielded bundles and take the witnessed form
/// (`zebra_chain::transaction::unmined`), so they are covered by the first case. Their txid and
/// authorizing-data digest both run through librustzcash, whose crosslink digester also folds in
/// the transaction's `staking_action` — so two transactions that differ only in their staking
/// action do not share a key.
///
/// The sighash is named separately because it is not always a function of the transaction alone.
/// A v5, v6 or v7 sighash also commits to the amounts and `scriptPubKey`s of the spent transparent
...
///
/// The verifying key is absent on purpose: this node has one Orchard verifying key and one
/// Sapling key pair for all of history, so an entry is only ever read back under the key it was
/// written against. A node that gains a second Orchard circuit era must partition the caches by
/// era, as Zakura does, or add the verifying key here.
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
...
///
/// This type is public only because it appears in the public verifier signatures. The private
/// `cache` module is not re-exported, and its constructor and accessors are private.
pub struct Cached<S> {
