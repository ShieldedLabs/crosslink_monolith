...
/// Returns the Halo2 *verifying key* for the Orchard bundles of a block at `network_upgrade`.
///
/// This is the key behind [`orchard_v5_verifier_for`], exposed directly for callers that drive
/// `orchard::bundle::BatchValidator` themselves: from orchard 0.15 the key is fixed when the batch
/// is constructed (`BatchValidator::new(&key)`) rather than passed to `validate()`, so the era must
/// be resolved up front.
///
/// Selecting by era rather than hard-coding a key matters for any chain that is not yet at NU6.3 --
/// Crosslink's own testnets run at NU6, whose Orchard proofs do not verify under the NU6.3 key.
/// At NU6.3 onward this returns the same key as [`orchard_v6_verifier`], so v6 Orchard and Ironwood
/// bundles are unaffected.
pub fn orchard_verifying_key_for(network_upgrade: NetworkUpgrade) -> &'static ItemVerifyingKey {
    use NetworkUpgrade::*;

    match network_upgrade {
        Genesis | BeforeOverwinter | Overwinter | Sapling | Blossom | Heartwood | Canopy | Nu5
        | Nu6 | Nu6_1 => &VERIFYING_KEY_PRE_NU6_2,
        Nu6_2 => &VERIFYING_KEY_NU6_2,
        Nu6_3 | Nu7 => &VERIFYING_KEY_NU6_3_ONWARD,
        #[cfg(zcash_unstable = "zfuture")]
        ZFuture => &VERIFYING_KEY_NU6_3_ONWARD,
    }
}
