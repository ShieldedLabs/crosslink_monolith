...
const AUTO_SPEND: u8 = 0; // automatically make spends without requiring GUI interaction
...
use zcash_primitives::transaction::{
    ...
    StakingAction_WithdrawDelegationBond, Transaction, TransactionData, TxVersion, Unauthorized,
};
...
impl WalletTx {
    ...
    pub fn fee(&self) -> Zatoshis {
        let all = self.totals();
        // NOTE: into_i64 isn't pub...
        let fee: u64 = (all.spent_zats.into_u64() as i64 - all.sent_zats.into_u64() as i64)
            .try_into()
...
impl WalletState {
    ...
    pub fn send_to_address(&mut self, address: String, amount: u64) {
        println!("SENDTOADDRESS: send_to_address");
...
fn update_with_tx(
    ...
    debug_assert!(
        new_totals.sent_zats == Zatoshis::const_from_u64(0)
...
impl ManualWallet {
    ...
    pub async fn send_zats<P: Parameters>(
        ...
        let fee_zats = 5 * zip317::MARGINAL_FEE.into_u64(); // NOTE: has headroom; TODO: actually calculate
        ...
        //- SPENDS
        let min_spend = t_send_z + s_send_z + fee_zats;
    ...
    pub async fn shield_transparent_zats<P: Parameters>(
        ...
        let memo_bytes =
            MemoBytes::from_bytes("shielding notes (fn shield_transparent_zats)".as_bytes())
                .unwrap();
        memo_count = 1;
    ...
    pub async fn stake_orchard_to_finalizer<P: Parameters>(
        ...
        println!("staking with orchard anchor at {orchard_anchor_h:?}: {orchard_anchor:?}");
        ...
        println!("  added stake output: {}", exact_amount_to_send);
    ...
    pub async fn begin_unbonding_using_orchard<P: Parameters>(
        &mut self,
        network: P,
        client: &mut CompactTxStreamerClient<Channel>,
        src_usk: &UnifiedSpendingKey,
        ...
        if s_spend_z < rolling_fee_estimate {
            println!(
                "tx build error: not enough unspent orchard notes, got {} zats needed {}",
                s_spend_z, rolling_fee_estimate
            );
            return None;
        }
    ...
    pub async fn claim_bond_using_orchard<P: Parameters>(
        ...
        let bond_value = self
            ...
            .find(|t| {
                t.staking_action
                    .filter(|s| s.kind == StakingActionKind::CreateNewDelegationBond)
                    .map(|s| s.arg32_0)
                    .unwrap_or_default()
                    == *bond_key
            })
...
fn read_full_tx(
    ...
    if let Some(staking_action) = tx.staking_action() {
        if staking_action.kind == StakingActionKind::CreateNewDelegationBond {
            t_send_z += staking_action.amount_zats;
        }
        if staking_action.kind == StakingActionKind::WithdrawDelegationBond {
            t_spend_z += staking_action.amount_zats;
        }
    }
    ...
    'outer_sync: loop {
        ...
        //-- SEND DATA TO UI
        {
            ...
            for tx in &user_wallet.txs {
                if let Some(staking_action) = (&tx.staking_action) {
                    ...
                    {
                        stake_positions_bonded
                            .push((create_bond.unique_pubkey, create_bond.amount_zats));
                    }
                    if let Some(unbond) =
                        StakingAction_BeginDelegationUnbonding::try_from_union(staking_action)
                    {
                        ...
                        {
                            stake_positions_unbonded
                                .push((unbond.unique_pubkey, stake_positions_bonded[existing_i].1));
        ...
        if AUTO_SPEND != 0 {
            ...
            if user_wallet.accounts[0].unspent_orchard_notes.len() == 0 {
                ...
                let keys = PreparedKeys::from_ufvk_all(&miner_wallet.accounts[0].ufvk);
                let output = &[TxOutput::Orchard {
                    ovk: Some(keys.orchard_ovk.unwrap()), // TODO: allow default of sender ovk
                    dst: *user_ua.orchard().unwrap(),
                ...
                match &action {
                    ...
                    WalletAction::StakeToFinalizer(amount, target_finalizer) => {
                        ...
                        let maybe_send_txid = user_wallet
                            ...
                            .await;
                        println!("Try stake txid: {:?}", maybe_send_txid);
                        match maybe_send_txid {
                            None => {
                    ...
                    WalletAction::SendToAddress(address, amount) => {
                        println!("SendToAddress: {}", amount.into_u64());
                        let (user_wallet, miner_wallet) = (
                            &mut user_wallets[user_use_i],
                            &mut miner_wallets[miner_use_i],
                        );
